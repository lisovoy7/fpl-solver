...
def _get_gw_data(bootstrap: dict) -> pd.DataFrame:
    """Return cached gw_data or fetch fresh from FPL API."""
...
async def optimize(req: OptimizeRequest):
    ...
    try:
        ...
        predictions = generate_predictions(
            gw_data, fixtures, multipliers, current_season_tiers, season
        )

        # 7. Watchlist
        must_include = list(current_squad)
        must_include.extend(
            pid for pid, _ in [(pid, []) for pid in req.forced_lineup]
        )
        ...
        # 9. Free Hit pre-calc
        forced_lineup_tuples = [(pid, list(range(current_gw, current_gw + horizon)))
                                for pid in req.forced_lineup]
        fh_benefits: dict = {}
        ...
        for idx, scenario in enumerate(chip_scenarios, 1):
            ...
            solver.build_model()
            success = solver.solve(time_limit=req.time_limit_per_scenario)
            if not success:
                continue
            ...
            logger.info("[%d/%d] %-30s  %.1f pts", idx, len(chip_scenarios),
                        scenario["name"], total_points)
